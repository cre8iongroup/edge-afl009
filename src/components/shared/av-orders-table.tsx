...
export default function AVOrdersTable() {
  ...
  const pushFiltersToUrl = useCallback(
    (nextStatus: string[], nextType: string[], nextProxy: ProxyFilter) => {
  ...
  const toggleStatus = (v: string) => {
    ...
    pushFiltersToUrl(next, typeFilter, proxyFilter);
  };

  const toggleType = (v: string) => {
    const next = typeFilter.includes(v) ? typeFilter.filter(x => x !== v) : [...typeFilter, v];
    setTypeFilter(next);
    pushFiltersToUrl(statusFilter, next, proxyFilter);
  };
  ...
  const filtered = useMemo(() =>
    ...
    [enriched, statusFilter, typeFilter, proxyFilter],
  );
    ...
    <TooltipProvider delayDuration={200}>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border bg-muted/30 px-4 py-3">
        ...
        <ThreeWay label="Proxy" value={proxyFilter} onChange={(v) => {
          setProxyFilter(v);
          pushFiltersToUrl(statusFilter, typeFilter, v);
        }} />
